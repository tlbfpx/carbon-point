...
export const getProducts = async (params: ProductQuery) => {
  ...
  const res = await platformApiClient.get('/products', { params: queryParams });
  return res.data;
};
...
export const toggleProductStatus = async (id: string, _status: 'active' | 'inactive') => {
  const res = await platformApiClient.put(`/products/${id}/toggle`);
  return res.data;
};

export const updateStock = async (id: string, stock: number) => {
  const res = await platformApiClient.put(`/products/${id}/stock`, { delta: stock });
  return res.data;
};
