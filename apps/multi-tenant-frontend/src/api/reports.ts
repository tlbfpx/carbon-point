...
export const getDashboardStats = async (_tenantId: string) => {
  const res = await platformApiClient.get('/reports/report/dashboard/stats');
  return res.data;
};

export const getCheckInTrend = async (_tenantId: string, days = 7) => {
  const res = await platformApiClient.get('/reports/report/dashboard/checkin-trend', {
    params: { days },
  });
  return res.data;
};
...
export const getPointsTrend = async (_tenantId: string, days = 7) => {
  const res = await platformApiClient.get('/reports/report/dashboard/points-trend', {
    params: { days },
  });
  return res.data;
};
...
export const getHotProducts = async (_tenantId: string, limit = 5) => {
  const res = await platformApiClient.get('/reports/report/dashboard/hot-products', {
    params: { limit },
  });
  return res.data;
};
