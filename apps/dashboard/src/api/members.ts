...
export const getMembers = async (params: MemberQuery) => {
  const res = await apiClient.get('/system/users', { params });
  return res.data;
};

export const createMember = async (data: { phone: string; username: string; tenantId?: string }) => {
  const res = await apiClient.post('/system/users', data);
  return res.data;
};

export const updateMember = async (userId: string, data: Partial<Member>) => {
  const res = await apiClient.put(`/system/users/${userId}`, data);
  return res.data;
};

export const toggleMemberStatus = async (userId: string, status: 'active' | 'inactive') => {
  const res = await apiClient.put(`/system/users/${userId}/status`, { status });
  return res.data;
};

export const importMembers = async (formData: FormData) => {
  const res = await apiClient.post('/system/users/import', formData, {
...
export const generateInviteLink = async (userId: string) => {
  const res = await apiClient.post(`/system/users/${userId}/invite-link`);
  return res.data;
};
