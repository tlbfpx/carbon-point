...
-- ============================================================
-- DDL 增量：添加 level_mode 字段（2026-04-13）
-- ============================================================
ALTER TABLE tenants ADD COLUMN level_mode VARCHAR(20) NOT NULL DEFAULT 'strict'
    COMMENT '等级模式: strict(只升不降)/flexible(每月降级)' AFTER updated_at;

-- ============================================================
-- 通知模板：等级降级通知（2026-04-13）
-- ============================================================
INSERT INTO notification_templates (type, channel, title_template, content_template, is_preset) VALUES
('level_down', 'in_app', '等级降级通知', '您的等级已从 Lv.{old_level} 降为 Lv.{new_level}（{level_name}），积分系数调整为 {coefficient}x。继续加油打卡，重回巅峰！', 1)
ON DUPLICATE KEY UPDATE title_template = title_template;
