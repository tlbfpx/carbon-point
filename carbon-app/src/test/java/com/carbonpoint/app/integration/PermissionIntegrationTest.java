...
class PermissionIntegrationTest extends BaseIntegrationTest {
    ...
    void testUnauthorizedAccessRejected() throws Exception {
        ...
        testDataHelper.tenant("权限测试租户").id(701L).save();
        User user = testDataHelper.user(701L, "13800000701", "Test@123")
                .id(701L)
                .save();

        String token = generateToken(user.getId(), 701L, List.of("regular_user"));
    ...
    void testPermissionBasedAccessControl() throws Exception {
        // Create tenant
        testDataHelper.tenant("权限测试租户B").id(702L).save();

        // Create admin role with permissions
        Role adminRole = new Role();
        adminRole.setId(702L);
        adminRole.setTenantId(702L);
        ...
        regularRole.setId(703L);
        regularRole.setTenantId(702L);
        regularRole.setName("普通用户");
        regularRole.setIsPreset(false);
        roleMapper.insert(regularRole);

        // Admin user
        User adminUser = testDataHelper.user(702L, "13800000702", "Test@123")
                .id(702L)
                .save();

        // Regular user
        User regularUser = testDataHelper.user(702L, "13800000703", "Test@123")
                .id(703L)
                .save();
        ...
        setTenantContext(702L);
        String adminToken = generateToken(adminUser.getId(), 702L, List.of("admin"));
        String createUserJson = """
            {
                "phone": "13900000701",
                "password": "Test@123",
        ...
        // Regular user tries to create a user → should be rejected
        setTenantContext(702L);
        String regularToken = generateToken(regularUser.getId(), 702L, List.of("regular_user"));
        MvcResult regularResult = postJson("/api/users", createUserJson, regularToken);
    ...
    void testSuperAdminCannotBeDeleted() throws Exception {
        ...
        testDataHelper.tenant("预设角色测试租户").id(703L).save();
        Role presetAdmin = new Role();
        presetAdmin.setId(704L);
        presetAdmin.setTenantId(703L);
        presetAdmin.setName("超级管理员");
        presetAdmin.setIsPreset(true);
        roleMapper.insert(presetAdmin);

        User admin = testDataHelper.user(703L, "13800000704", "Test@123")
                .id(704L)
                .save();
        ...
        String token = generateToken(admin.getId(), 703L, List.of("super_admin"));

        // Try to delete the preset admin role
        setTenantContext(703L);
