...
class LoginSecurityTest extends BaseIntegrationTest {
    ...
    void testLoginAttemptsAreLogged() throws Exception {
        ...
        testDataHelper.tenant("登录日志测试租户").id(801L).save();
        User user = testDataHelper.user(801L, "13900000801", "Test@123")
                .id(801L)
                .status("active")
                .save();
        ...
        wrapper.eq(LoginSecurityLogEntity::getUsername, "13900000801")
    ...
    void testLoginWithLockedAccountRejected() throws Exception {
        testDataHelper.tenant("锁定账户测试租户").id(901L).save();
