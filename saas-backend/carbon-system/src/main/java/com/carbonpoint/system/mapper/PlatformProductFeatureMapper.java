...
@Mapper
@InterceptorIgnore
public interface PlatformProductFeatureMapper extends BaseMapper<PlatformProductFeature> {

    @Select("SELECT * FROM product_features WHERE product_id = #{productId}")
    List<PlatformProductFeature> selectByProductId(@Param("productId") String productId);

    @Select("SELECT * FROM product_features WHERE product_id = #{productId} AND feature_id = #{featureId}")
    PlatformProductFeature selectByProductIdAndFeatureId(@Param("productId") String productId, @Param("featureId") String featureId);
