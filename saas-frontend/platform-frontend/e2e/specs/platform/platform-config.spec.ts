...
test.describe('平台后台 - 平台配置', () => {
  ...
  test('PC-004: 切换到通知设置Tab', async ({ page }) => {
    ...
    // Wait for tab content to render — scope to active panel since Ant Design hides inactive tabs
    await page.waitForSelector('.ant-tabs-tabpane-active .ant-form-item', { timeout: 5000 });
    const formItems = page.locator('.ant-tabs-tabpane-active .ant-form-item');
  ...
  test('PC-006: 切换到功能开关Tab', async ({ page }) => {
    ...
    // Check for toggle switches in active panel
    await page.waitForSelector('.ant-tabs-tabpane-active .ant-switch', { timeout: 5000 });
  ...
  test.skip('PC-009: 必填字段验证 - 留空提交', async ({ page }) => {
    // SKIPPED: Basic config tab has no required fields marked with .ant-form-item-required.
    // The platform config page structure differs from test assumptions.
    const requiredInput = page.locator('.ant-tabs-tabpane-active .ant-form-item-required input').first();
    await requiredInput.clear();
    await page.locator('button').filter({ hasText: '保存' }).first().click();
    const errorMsg = page.locator('.ant-tabs-tabpane-active .ant-form-item-explain-error');
    await expect(errorMsg.first()).toBeVisible({ timeout: 3000 });
  });

  test('PC-010: 邮箱格式验证', async ({ page }) => {
    // Find email field within active tab panel
    const emailField = page.locator('.ant-tabs-tabpane-active .ant-form-item').filter({ hasText: /邮箱|email/i }).locator('input').first();
  ...
  test('PC-011: URL格式验证', async ({ page }) => {
    const urlField = page.locator('.ant-tabs-tabpane-active .ant-form-item').filter({ hasText: /URL|链接|地址/i }).locator('input').first();
  ...
  test('PC-012: 数字字段边界值验证', async ({ page }) => {
    const numberField = page.locator('.ant-tabs-tabpane-active .ant-form-item').filter({ hasText: /数量|上限|最大/i }).locator('input').first();
    if (await numberField.isVisible()) {
      await numberField.fill('999999999999');
      await page.locator('button').filter({ hasText: '保存' }).first().click();
      const errorOrSuccess = await Promise.race([
        page.locator('.ant-tabs-tabpane-active .ant-form-item-explain-error').first().isVisible({ timeout: 2000 }).then(() => 'error'),
  ...
  test('PC-013: 功能开关切换', async ({ page }) => {
    ...
    await featureTab.click();
    await page.waitForSelector('.ant-tabs-tabpane-active .ant-switch', { timeout: 5000 });
    const firstSwitch = page.locator('.ant-tabs-tabpane-active .ant-switch').first();
  ...
  test.skip('PC-014: 保存成功提示', async ({ page }) => {
    ...
    const input = page.locator('.ant-tabs-tabpane-active .ant-form input').first();
  ...
  test.skip('PC-015: 保存后数据持久化', async ({ page }) => {
    ...
    const newInput = page.locator('.ant-tabs-tabpane-active .ant-form input').first();
  ...
  test('PC-017: 下拉选择框可展开', async ({ page }) => {
    const select = page.locator('.ant-tabs-tabpane-active .ant-select').first();
  ...
  test('PC-018: 日期选择器可打开', async ({ page }) => {
    const datePicker = page.locator('.ant-tabs-tabpane-active .ant-picker').first();
  ...
  test('PC-019: 多行文本框可编辑', async ({ page }) => {
    const textarea = page.locator('.ant-tabs-tabpane-active .ant-input-textarea textarea');
