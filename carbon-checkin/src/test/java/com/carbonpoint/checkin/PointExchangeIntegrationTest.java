...
class PointExchangeIntegrationTest extends BaseIntegrationTest {
    ...
    /** Extract int from map (mirrors PointAccountService.getInt). */
    ...
    void testExchangeCoupon() throws Exception {
        ...
        Map<String, Object> updatedUser = pointsUserMapper.selectById(user.getId());
        assertNotNull(updatedUser, "User should be retrievable after exchange");
        assertEquals(400, getInt(updatedUser, "available_points"),
                "Available points should decrease by product price");
        // Total points decreases: freezePoints uses updatePointsAtomic(delta=-100)
        // which reduces total_points by 100 (points are "consumed" from total, moved to frozen)
        assertEquals(400, getInt(updatedUser, "total_points"),
                "Total points should decrease by product price (frozen from total)");

        // Step 5: Verify order was created
        setTenantContext(2001L); // Ensure tenant context is set for mapper queries
    ...
    void testSoldOutProductRejected() throws Exception {
        ...
        Map<String, Object> updatedUser = pointsUserMapper.selectById(user.getId());
        assertNotNull(updatedUser, "User should be retrievable");
        assertEquals(5000, getInt(updatedUser, "available_points"),
    ...
    void testExchangeCreatesPointTransaction() throws Exception {
        ...
        // Verify point transaction was created (freezePoints creates type "frozen")
        PointTransactionEntity tx = pointTransactionMapper.selectOneByUserIdAndType(user.getId(), "frozen");

        assertNotNull(tx, "Point transaction should be created for exchange");
        assertEquals(-200, tx.getAmount(),
                "Transaction amount should be negative (frozen deduction)");
